        ...
        <section id="how-it-works" className={sectionBorder}>
          <div className="py-16 flex flex-col items-center text-center px-4">
            ...
            <motion.div ref={howItWorksRef} initial={{ opacity: 0, y: 30 }} animate={howItWorksInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}>
              ...
              <p className="mt-3 text-[#605A57] dark:text-[#A8A29E] text-sm sm:text-base max-w-md mx-auto">Create and deploy your custom chatbot in minutes.</p>
            </motion.div>
          </div>

          <div className="flex">
            <HatchStrip />
            <div className="flex-1 flex flex-col md:flex-row border-l border-r border-[rgba(55,50,47,0.12)] dark:border-[#44403C]">
              {[
                { step: "01", title: "Create & Configure", desc: "Craft your chatbot by inputting custom Q&As and defining conversation flows.", icon: <Settings className="w-5 h-5 text-[#49423D] dark:text-[#F5F5F4]" /> },
                { step: "02", title: "Customize Design", desc: "Tailor your bot's appearance to match your brand — colors, fonts, and positioning.", icon: <Paintbrush className="w-5 h-5 text-[#49423D] dark:text-[#F5F5F4]" /> },
                { step: "03", title: "Plug & Play", desc: "Get your unique embed code and add the chatbot to your website instantly.", icon: <Code className="w-5 h-5 text-[#49423D] dark:text-[#F5F5F4]" /> },
              ].map((s, i) => (
                <div key={i} className={`flex-1 p-6 sm:p-8 flex flex-col gap-3 ${i < 2 ? "border-b md:border-b-0 md:border-r" : ""} border-[rgba(55,50,47,0.12)] dark:border-[#44403C]`}>
                  <div className="flex items-center gap-3">
                    <span className="text-[rgba(55,50,47,0.30)] dark:text-[#A8A29E]/30 text-4xl font-serif">{s.step}</span>
                    <div className="w-8 h-8 rounded-lg bg-[rgba(55,50,47,0.06)] dark:bg-[#44403C] flex items-center justify-center">{s.icon}</div>
                  </div>
                  <h3 className="text-[#49423D] dark:text-[#F5F5F4] text-sm font-semibold">{s.title}</h3>
                  <p className="text-[#605A57] dark:text-[#A8A29E] text-[13px] leading-[22px]">{s.desc}</p>
                </div>
              ))}
            </div>
            <HatchStrip />
        ...
        <motion.section ref={faqRef} initial={{ opacity: 0, y: 40 }} animate={faqInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} id="faq" className={sectionBorder}>
          <div className="py-16 px-4">
            <div className="flex flex-col items-center text-center mb-12">
              <Badge text="FAQ" />
              <h2 className="mt-5 text-2xl sm:text-3xl lg:text-5xl font-semibold tracking-tight text-[#49423D] dark:text-[#F5F5F4] font-sans">Frequently Asked Questions</h2>
              <p className="mt-3 text-[#605A57] dark:text-[#A8A29E] text-sm sm:text-base max-w-md">Everything you need to know about Askio</p>
            </div>
            ...
            <div className="max-w-[700px] mx-auto space-y-3">
              {[
                ...
                <div key={index} className="bg-white dark:bg-[#292524] rounded-lg border border-[#E0DEDB] dark:border-[#44403C] overflow-hidden hover:border-[rgba(55,50,47,0.30)] dark:hover:border-[#57534E] transition-colors">
                  <button className="w-full px-6 py-5 text-left flex justify-between items-center" onClick={() => toggleFAQ(index)} aria-expanded={activeIndex === index}>
                    <span className="text-[#37322F] dark:text-[#F5F5F4] text-sm sm:text-base font-semibold pr-4">{faq.q}</span>
                    <motion.div animate={{ rotate: activeIndex === index ? 180 : 0 }} transition={{ duration: 0.3 }} className="text-[#605A57] dark:text-[#A8A29E] shrink-0">
                      <ChevronDown className="w-5 h-5" />
                    </motion.div>
                  </button>
                  <AnimatePresence initial={false}>
                    {activeIndex === index && (
                      <motion.div key={`a-${index}`} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }}>
                        <div className="px-6 pb-5">
                          <p className="text-[#605A57] dark:text-[#A8A29E] text-sm leading-relaxed border-l-2 border-[#E0DEDB] dark:border-[#44403C] pl-4">{faq.a}</p>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
